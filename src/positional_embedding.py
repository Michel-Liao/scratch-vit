...
class PositionalEmbedding:
    ...
    def __init__(self, num_patches: int, embedding_dim: int, init: str = "normal") -> None:
        """
        ...
        if init == "normal":
            self.pos_embedding = cp.random.normal(0, 0.02, (1, num_patches, embedding_dim))
        elif init == "uniform":
            self.pos_embedding = cp.random.uniform(-0.02, 0.02, (1, num_patches, embedding_dim))
        else:
    ...
    def forward(self, x: cp.ndarray) -> cp.ndarray:
        ...
        if x.shape[1:] != (self.num_patches, self.embedding_dim):
            raise ValueError(f"Expected input shape [batch_size, {self.num_patches}, {self.embedding_dim}], got {x.shape}")
    ...
    def update_params(self) -> None:
        ...
        """
        self.pos_embedding = self.optimizer.update(self.grad_pos_embedding, self.pos_embedding)
