...
class MultiHeadAttention:
    ...
    def backward(self, error: cp.ndarray) -> cp.ndarray:
        ...
        error_head_split = cp.stack(
            cp.split(error, self.n_heads, axis=-1), axis=0
        )  # (n_heads, batch, seq_len, d_head)

        # Compute gradients for each head in parallel
        err_attn = cp.matmul(
            error_head_split, self.v_seqs.transpose(0, 1, 3, 2)
        )  # (n_heads, batch, seq_len, seq_len)
        pre_attn_error = cp.stack(
            [softmax.backward(err) for softmax, err in zip(self.softmax, err_attn)],
            axis=0,
        )

        # Compute gradients for V
        v_grad_in = cp.matmul(
            self.attention_seqs.transpose(0, 1, 3, 2), error_head_split
        )  # (n_heads, batch, seq_len, d_head)
        error_v_out = cp.stack(
            [
                v_mapping.backward(vg)
                for v_mapping, vg in zip(self.v_mappings, v_grad_in)
            ],
            axis=0,
        )
        ...
        k_error = (
            cp.matmul(self.q_seqs.transpose(0, 1, 3, 2), pre_attn_error) / self.scale
        )  # (n_heads, batch, seq_len, d_head)
        error_k_out = cp.stack(
            [
                k_mapping.backward(k_err.transpose(0, 1, 3, 2))
                for k_mapping, k_err in zip(self.k_mappings, k_error)
            ],
            axis=0,
        )
        ...
        q_error = (
            cp.matmul(pre_attn_error, self.k_seqs) / self.scale
        )  # (n_heads, batch, seq_len, d_head)
        error_q_out = cp.stack(
            [
                q_mapping.backward(q_err)
                for q_mapping, q_err in zip(self.q_mappings, q_error)
            ],
            axis=0,
        )
        ...
        return cp.concatenate(
            cp.split(seq_error, self.n_heads, axis=0), axis=-1
        ).squeeze(
            0
